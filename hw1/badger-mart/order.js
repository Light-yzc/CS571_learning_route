...
function calculateSubtotal() {
	let sum = 0;
	for (let i of ITEMS){
		const quantity = parseInt(document.getElementById(i + "-quantity").value);
		const price = parseFloat(document.getElementById(i + "-price").innerText);
		console.log(quantity)
		sum += price * quantity	
	}
	return sum; // TODO calculateSubtotal

}

function calculateSalesTax() {
	const tax = calculateSubtotal();
	const tax_tax = getSalesTaxRateForState()
	return roundMoney(tax * tax_tax)
	// return 0.00; // TODO calculateSalesTax
}
function getSalesTaxRateForState(){
	const state1 = document.getElementById("state-tax").value;
	return SALES_TAX[state1]
}
